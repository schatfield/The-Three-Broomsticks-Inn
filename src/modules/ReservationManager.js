...
export default {
  get(reservationId) {
    return fetch(
      `${remoteURL}/reservations/${reservationId}?embed=rooms`
  ...
  getReservationByUserId(userId) {
    return fetch(
      `${remoteURL}/reservations?userId=${userId}&_expand=room`
    ).then((result) => result.json());
  },
  ...
  getSelectedServicesByReservationId(id) {
    return fetch(
      `${remoteURL}/reservationServices?reservationId=${id}&isSelected=true&_expand=service`
    ).then((result) => result.json());
  },

  getServicesByReservationId(id) {
    return fetch(
      `${remoteURL}/reservationServices?reservationId=${id}&_expand=service`
    ).then((result) => result.json());
  },

  getRoomImageByRoomId(id) {
    return fetch(`${remoteURL}/roomImages?roomId=${id}`).then((result) =>
      result.json()
    );
  },
};
